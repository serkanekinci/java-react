...
export default class User extends ShallowComponent {
    ...
    __edit() {
        ...
        selectedRows[0].roleOid=this.__findRoleObject(selectedRows[0].roleGroup.oid).oid;
    ...
    __onSave(newData, callback) {
        ...
        newData.roleGroup = this.__findRoleObject(newData.roleOid);
    ...
    __cellRenderer(idx: number, fields: Array, row: Object) {
        ...
        if (fields[idx].name == 'roleOid') {
            return <td key={fields[idx].name}>{row.roleGroup.name}</td>;
        }
